...
app.post('/orders', (req, res) => {
  ...
  db.get(checkAvailabilityQuery, [product_id], (err, row) => {
    ...
    const queryOrder = 'INSERT INTO Orders (user_id, quantity, order_number, delivery_time, status_id, address, courier_id, user_comment) VALUES (?, ?, ?, ?, ?, ?, ?, ?)';
    db.run(queryOrder, [user_id, quantity, order_number, delivery_time, status_id, address, courier_id, user_comment], function (err) {
        if (err) {
            res.status(500).json({ error: err.message });
            return;
        }
